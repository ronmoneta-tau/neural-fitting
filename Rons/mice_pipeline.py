...
def create_bound_maps(solute_data: DataConfig, roi_config: ROIConfig, sli:int=0) -> None:
    ...
    fig, axes = au.plot_CI_maps(solute_data.f_values[:,sli,:] * 100, f_sigma[:,sli,:], solute_data.k_values[:,sli,:], k_sigma[:,sli,:], is_mt=(solute_data.name == 'MT'))
    ...
    print("f_val stats (min, mean, max):", np.nanmin(solute_data.f_values) * 100, np.nanmean(solute_data.f_values) * 100, np.nanmax(solute_data.f_values) * 100)
    ...
    for jj, (_x, _y) in enumerate(roi_config.points):
        if auxiliary_mt_data is not None:
            f_best_dotprod, k_best_dotprod, nrmse, _df, _dk = au.get_nrmse_grid(
                ...
                loc_dict_res=100,max_f_mt=.3, max_k_mt=110,max_f_amide=0.02, max_k_amide=200,
            ...
            f_best_dotprod, k_best_dotprod, nrmse, _df, _dk = au.get_nrmse_grid(
                ...
                loc_dict_res=100, max_f_mt=.3, max_k_mt=110,
        ...
        plt.suptitle(f'{solute_data.name} posterior distribution\n{roi_name} Point: ({_x},{_y}). CR_area: {CR_area:.2f}', fontsize=16)
        plt.tight_layout()
...
def main():
    ...
    rNOE_data = DataConfig('rNOE', data_xa_cutout, inpt, figs_path)
