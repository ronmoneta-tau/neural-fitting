...
def wassr_b0_mapping(Z_Ims_3DMat,
    ...
    broadcasted_mask = np.broadcast_to(Brain_Mask[:, :, np.newaxis],
                                       Z_Ims_3DMat.shape)  # Broadcast the mask to all channels
        ...
        for c_ind in range(B0Map.shape[1]):
            ...
            if Brain_Mask[r_ind, c_ind]:
                ...
                try:
                    ...
                    par, _ = curve_fit(lorentz_iN, x_data, y_data, p0=initial_guess,
                                       bounds=(bounds_lower, bounds_upper))
                except RuntimeError:
                    # Handle the RuntimeError
                    print(f'Problem pixel for r: {r_ind}, c: {c_ind}')
...
def z_spec_rearranger(z_spec_data):
    ...
    z_spec_rearr[last_i:mid_i:-1, :, :] = z_spec_data[2:-1:2, :,
                                          :]  # negatives (by order) [-0.25, ...,-6.75,-7], (25/28, 64, 64)
