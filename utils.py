...
def myplot1(data, bcrop=0, tcrop=0, rotate=False, log_hist=False, 
    ...
    lims = np.percentile(data_flat[~np.isnan(data_flat)], [range_perc_crop, 100-range_perc_crop]) if lims is None else lims
    ...
    dataf = data.flatten()
    z=ax_histx.hist(data_crop_rot_clipped.flatten(), np.linspace(lims[0], lims[1], 32), orientation='horizontal', alpha=0.5, log=log_hist)
...
def plot_estimation_scatter(fss_gt_np, kss_gt_np, pred_fss_np, pred_kss_np, roi_mask_zeros,
    ...
    plt.plot((fss_gt_np*roi_mask_zeros).flatten(), (pred_fss_np*roi_mask_zeros).flatten(), '.', alpha=alpha)  # [::ds,rs,::ds]
    plt.grid()
...
def larg_phantom_viz(
    ...
    if Ksw_Hz_GT_QUESP is not None:
        ...
        for vi, ((y,x), gt_ph, gt_Hz, gt_Hz_std) in enumerate(zip(centers, PH, Ksw_Hz_GT_QUESP, Ksw_Hz_GT_QUESP_std)): 
            ...
            if False:
                plt.text(x, y-5, f'{gt_Hz:.0f}±{gt_Hz_std:.0f}', color=[1,1,1], fontsize=10, ha='center', va='center')
...
def slice_row_plot(fss_pred, kss_pred, err_3d, slices=None,
    ...
    for slind, slice2plot in enumerate(slices):
        ...
        plt.gca().margins(0,0); plt.gca().set_xticklabels([]); plt.gca().set_yticklabels([])
        ...
        if do_err:
            ...
            plt.gca().margins(0,0); plt.gca().set_xticklabels([]); plt.gca().set_yticklabels([]); plt.axis('off')
            if slice2plot == slices[-1]:        
                cax = ax.inset_axes(clrbr_dims);    
                fig.colorbar(imshow_res, cax=cax, orientation='vertical') 
...
def show_correspondence_stats_boxplots(
    ...
    xlims = {
        ...
        "MAPE (%)": {'lims': [0, 100]}, 
        "ICC(2,1)": {'lims': [0, 1], 'ticks': [0,0.25,0.5,0.75,1], 'ticklabels': ['0','0.25','0.5','0.75','1']}, 
        }, 
    figsize=(25, 3)
    ):    
...
def plot_basic_brain_stats(volunteers):
    ...
    fig, axes = plt.subplots(1, len(parameters), figsize=(16, 6), sharey=True) #, gridspec_kw={'width_ratios': [1, 1, 1]})
...
def slice_row_plot_auxiliary(data, figsize=None, slices=None, t2ms_lims=None):
    ...
    for slind, slice2plot in enumerate(slices):
        ...
        ax.spines[:].set_visible(False); ax.tick_params(left=False, right=False, top=False, bottom=False)
        ax = plt.subplot(rows, cols, cols+1+slind)    
        plt.gca().margins(0,0); plt.gca().set_xticklabels([]); plt.gca().set_yticklabels([])
...
def plot_XY_bland_altman(
    ...
    # 2. Bland-Altman Plot
    ax2 = axes[1]
    sns.scatterplot(x=means, y=differences, s=30, color="teal", ax=ax2, alpha=alpha)#, label=f"{param_name} (Bland-Altman)")
    ...
    xlim = ax1.get_xlim(); diff = xlim[1]-xlim[0]
...
def robustness_boxplot_train_test(
    ...
    plt.xticks(ticks=range(len(labels)), labels=[f'run {i//2+1}' if i % 2 == 0 else '' for i in range(len(labels))], rotation=45)
    plt.ylabel('NRMSE')
...
def pack_circles(image, centers, radius):
    ...
    for i, (x_center, y_center) in enumerate(centers):
        ...
        if (i % 2) == 1:
            ...
            roi_nan[:circle_diameter, i * radius: (i + 2) * radius] = packed_image[:circle_diameter, i * radius: (i + 2) * radius]            
        else: 
...
def GMWM_T1T2_boxplot(data_xa):
    ...
    box1wm = ax1.boxplot(t1_wm_values, positions=[0.8], widths=0.4, patch_artist=True, boxprops=dict(facecolor="lightblue"), whis=[2.5, 97.5])
    box1gm = ax1.boxplot(t1_gm_values, positions=[1.8], widths=0.4, patch_artist=True, boxprops=dict(facecolor="lightblue"), whis=[2.5, 97.5])
    ax1.set_ylabel('T1 (ms) ', color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')

    box2wm = ax2.boxplot(t2_wm_values, positions=[1.2], widths=0.4, patch_artist=True, boxprops=dict(facecolor="lightgreen"), whis=[2.5, 97.5])
    box2gm = ax2.boxplot(t2_gm_values, positions=[2.2], widths=0.4, patch_artist=True, boxprops=dict(facecolor="lightgreen"), whis=[2.5, 97.5])
...
def compare_fk_preds(bsf, estimated_params, pool='c'):
    ...
    plt.xlabel('true'); plt.ylabel('estimated')
    plt.title('f')
    plt.subplot(1,2,2)
